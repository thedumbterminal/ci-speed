...
def get_test_success(project_id, days):
    builds = query(
        (
            ...
            "CASE "
            "WHEN COUNT(*) FILTER (WHERE test_failure.test_case_id IS NOT NULL OR skipped_test.test_case_id IS NOT NULL) > 0 THEN "
            "CAST((( "
            "    CAST(COUNT(*) FILTER (WHERE test_failure.test_case_id IS NOT NULL OR skipped_test.test_case_id IS NOT NULL) AS DECIMAL) / "
            "    CAST(COUNT(*) FILTER (WHERE test_failure.test_case_id IS NULL AND skipped_test.test_case_id IS NULL) AS DECIMAL)"
            ") * 100) AS FLOAT) "
            "ELSE COUNT(*) FILTER (WHERE test_failure.test_case_id IS NOT NULL OR skipped_test.test_case_id IS NOT NULL) "
            "END AS percent "
            "FROM build "
